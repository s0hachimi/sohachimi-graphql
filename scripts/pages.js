...
export async function showProfile() {
    ...
    const LEVEL = level?.transaction?.[0]?.amount || 0
    const XP = xp?.transaction_aggregate?.aggregate?.sum?.amount || 0
    ...
    const objSkill = skillLevels[0] ? Object.entries(skillLevels[0]) : "You have no skills !"

    const divSkill = document.createElement("div")
    divSkill.setAttribute("class", "div-skill")

    if (objSkill && typeof objSkill === "string") {
        divSkill.textContent = objSkill
    ...
    const failed = audit?.user?.[0]?.failed?.length || 0
    const succeeded = audit?.user?.[0]?.succeeded?.length || 0
